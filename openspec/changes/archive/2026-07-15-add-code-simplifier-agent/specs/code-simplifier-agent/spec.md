# Spec: code-simplifier-agent

## ADDED Requirements

### Requirement: Agent is a valid OpenCode subagent definition
The agent SHALL be defined as a single markdown file with YAML frontmatter containing `description`, `mode: subagent`, and `temperature`, such that OpenCode registers it when the file is placed in `~/.config/opencode/agents/` or `.opencode/agents/`.
